    ...
    <div className="page-container">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {classes.length > 0 ? (
          ...
          <div className="classroom-card text-center py-8 text-muted-foreground text-sm">No classes found. Add one in Admin.</div>
        )}
          ...
          {moodHistory.length === 0 ? (
            ...
            <div className="space-y-3">
              {moodHistory.map((m: any) => {
                const entries = [{ e:"😊",c:m.happyCount,color:"bg-emerald-400" }, { e:"😐",c:m.neutralCount,color:"bg-amber-400" }, { e:"😢",c:m.sadCount,color:"bg-sky-400" }, { e:"😠",c:m.angryCount,color:"bg-rose-400" }];
                const total = entries.reduce((s, x) => s + x.c, 0) || 1;
                const dominant = [...entries].sort((a,b)=>b.c-a.c)[0];
                return (
                  <div key={m.id} className="p-4 rounded-2xl bg-secondary">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-black text-foreground">{new Date(m.date).toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric"})}</span>
                      <span className="text-xl">{dominant.e}</span>
                    </div>
                    <div className="flex rounded-full overflow-hidden h-2.5 gap-0.5">
                      {entries.map((e,i) => e.c > 0 && (
                        <div key={i} className={e.color + " rounded-full"} style={{ width:`${(e.c/total)*100}%` }} />
                      ))}
                    </div>
                    <div className="flex gap-3 mt-2">
                      {entries.map((e,i) => e.c > 0 && <span key={i} className="text-xs text-muted-foreground">{e.e} {e.c}</span>)}
                    </div>
                  </div>
                );
              })}

...
import { BookOpen, Play, CheckCircle2, Clock, Zap } from "lucide-react";
...
async function getSessionPageData() {
  ...
  const [classes, todaySessions, recentSessions] = await Promise.all([
    ...
    prisma.session.findMany({
      where: { date: today },
      include: { class: true, participation: { include: { student: true } }, activityEvents: true, teams: true },
...
export default async function SessionPage() {
  ...
  const totalXPToday = todaySessions.reduce((s: number, sess: any) =>
    s + sess.activityEvents.reduce((x: number, e: any) => x + e.xpAwarded + e.bonusXP, 0), 0);
        ...
        <div className="classroom-card">
          <h2 className="font-black text-foreground mb-4 flex items-center gap-2">
            ...
            {todaySessions.length > 0 && (
              <span className="ml-auto badge-pill bg-amber-100 text-amber-700">{todaySessions.length}</span>
            )}
          </h2>
          {todaySessions.length === 0 ? (
            <div className="text-center py-10">
              <p className="text-4xl mb-3">📖</p>
              <p className="text-muted-foreground text-sm">No sessions yet today.</p>
              <p className="text-xs text-muted-foreground mt-1">Create one to get started!</p>
            </div>
          ) : (
            <div className="space-y-3">
              {todaySessions.map((session: any) => {
                const sessionXP = session.activityEvents.reduce((s: number, e: any) => s + e.xpAwarded + e.bonusXP, 0);
                return (
                  <Link key={session.id}
                    href={`/session/${session.id}${!session.completed ? "/live" : "/summary"}`}
                    className="flex items-center gap-3 p-4 rounded-2xl border-2 border-transparent bg-secondary hover:border-violet-200 hover:bg-white hover:shadow-card transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {session.mode === "team" && <span className="text-xs font-bold text-amber-600">🏆 Team</span>}
                        {session.participation.length > 0 && <span className="text-xs text-muted-foreground">{session.participation.length} scored</span>}
                      </div>
                    </div>
            ...
            {recentSessions.map((session: any) => {
              ...
              return (
                <Link key={session.id} href={`/session/${session.id}/summary`}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-foreground truncate">{session.topic}</p>
                    <p className="text-xs text-muted-foreground">{session.class.name}</p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    {sessionXP > 0 && <span className="text-xs font-bold text-violet-500">+{sessionXP} XP</span>}
